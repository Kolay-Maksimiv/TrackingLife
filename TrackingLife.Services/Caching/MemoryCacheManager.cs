...
namespace TrackingLife.Services.Caching
{
    /// <inheritdoc cref="IStaticCacheManager" />
    /// <summary>
    ...
    public class MemoryCacheManager : ILocker, IStaticCacheManager
    {
        ...
        public T Get<T>(string key) => _provider.Get<T>(key).Value;
        ...
        /// <returns>The cached value associated with the specified key</returns>
        public T Get<T>(string key, Func<T> dataRetriever, int? cacheTime = null)
        {
            if (cacheTime <= 0)
                return dataRetriever();

            return _provider.Get(key, dataRetriever, TimeSpan.FromMinutes(cacheTime ?? CachingDefaults.CacheTime))
                .Value;
        }
        ...
        /// <param name="cacheTime">Cache time in minutes; pass 0 to do not cache; pass null to use the default time</param>
        /// <returns>The cached value associated with the specified key</returns>
        public async Task<T> GetAsync<T>(string key, Func<Task<T>> dataRetriever, int? cacheTime = null)
        {
            if (cacheTime <= 0)
                return await dataRetriever();

            var cacheValue = await _provider.GetAsync(key, dataRetriever, TimeSpan.FromMinutes(cacheTime ?? CachingDefaults.CacheTime));
